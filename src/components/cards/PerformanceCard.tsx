...
function DeltaColumn({ label, prevData, currentValue }: {
  label: string;
  prevData: PerfData | null;
  currentValue: number;
}) {
  ...
  return (
    <div>
      <div style={{
        ...
        letterSpacing: '0.5px', color: 'var(--text-m)', marginBottom: 5,
      }}>
        {label}
      </div>
      {hasData ? (() => {
...
export default function PerformanceCard({
  ...
  prevMonth, prevYear, highlight = false, delay = 0,
    ...
    >
      ...
      <div className="flex items-center justify-between mb-3">
        <span style={{
          ...
          letterSpacing: '0.6px', color: 'var(--text-s)',
        }}>
          {title}
        </span>
        <div
      ...
      {/* Primary value */}
      <div style={{
        fontSize: 22, fontWeight: 700, letterSpacing: '-0.7px',
        color: 'var(--text)', fontFamily: 'var(--mono)', marginBottom: 14,
      }}>
        {currentFormatted}
      </div>
      ...
      <div
        ...
        style={{ borderTop: '1px solid var(--border-l)', paddingTop: 12 }}
      >
        <DeltaColumn label="vs Mês Ant." prevData={prevMonth} currentValue={currentValue} />
        <DeltaColumn label="vs Ano Ant." prevData={prevYear} currentValue={currentValue} />
      </div>
    </div>
  );
}
